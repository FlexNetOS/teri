...
- **⚠️ OWNER DECISION QUEUED (`[!]`):** U-001 ported `LLM_MODEL_NAME` env correctly but teri's DEFAULT model stays `gpt-4o` vs MiroFish `gpt-4o-mini`. Surfaced as a defensible shimmy-target architect decision (config.rs:64-69), NOT silently accepted. If strict source-parity is wanted, change teri's default to `gpt-4o-mini`. Owner to confirm.
