...
- [x] S-367 · `unit:U-018` · `method` · `OasisProfileGenerator.generate_profiles_from_entities` · **PARITY-VERIFIED 2026-06-17 (opus gate).** realtime-vs-final split confirmed FAITHFUL (realtime=to_reddit/twitter_format serializers py:903/909↔rs:497/508; final=forced-default writers, both sides); sequential preserves entity-order (MiroFish pre-alloc ordered list); fallback dataclass defaults byte-match. **UN-[≠]'d DECISION-10 (contractual file/return output — re-opens U-018).** → `generate_profiles_from_entities<L: LlmClient>` (src/services/oasis_profile_export.rs) — sequential batch over `PersonaGenerator::generate_social`; per-entity fallback profile on error (bio="{type}: {name}", persona=summary-or-generic, user_id=idx); progress_callback(current,total,msg) after each; realtime-save hook via `realtime_output: Option<(&Path, OutputPlatform)>` (reddit→to_reddit_format JSON; twitter→to_twitter_format CSV; write-failure→tracing::warn+continue); returns ordered Vec in entity order. `_parallel_count` reserved for caller; this fn is sequential per DECISION-10 §4. Parity: 36 new tests pass (2026-06-17).
