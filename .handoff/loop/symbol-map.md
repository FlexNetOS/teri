...
- [x] S-619 · `unit:U-022` · `method` · `SimulationRunner.get_all_actions` · `simulation_runner.py:894` · verified-through-consumer in U-026 (h)/(i); empty-string platform falsy bug FIXED (no_filter) in (h).
- [x] S-620 · `unit:U-022` · `method` · `SimulationRunner.get_actions` · filter by platform/agent/round · `simulation_runner.py:955` · verified-through-consumer in U-026 (i) /actions (pagination + filters).
- [x] S-621 · `unit:U-022` · `method` · `SimulationRunner.get_timeline` · by rounds · `simulation_runner.py:989` · verified-through-consumer in U-026 (i) /timeline; **was NOT actually verified in U-022(d)** — (i) gate caught dropped first/last_action_time fields (9-key entry), FIXED.
- [x] S-622 · `unit:U-022` · `method` · `SimulationRunner.get_agent_stats` · action type distribution · `simulation_runner.py:1060` · verified-through-consumer in U-026 (i) /agent-stats; **was NOT actually verified in U-022(d)** — (i) gate caught action_types key-order + latent last_action_time never-updated, FIXED.
...
- [x] S-680 · `unit:U-023` · `method` · `SimulationManager.get_run_instructions` · `simulation_manager.py:507` → `SimulationManager::get_run_instructions` PARTIAL — returns `RunInstructions{simulation_dir, config_file, substrate_note}`. [≠]-substrate: `scripts_dir`, `commands` (Python OASIS subprocess invocations), and `instructions` (conda activate steps) are genuinely INEXPRESSIBLE in teri's substrate (teri has no `scripts/run_*.py` and no conda env; it runs SimEngine in-process). This is NOT "won't use" — the strings cannot be faithfully constructed. Structural fields (paths) ported; command strings omitted with `substrate_note` directing callers to `SimEngine::run`. **U-026-c EXTENSION (DECISION-U026-2) PARITY FAIL 2026-06-19 (opus):** additive change is correct in KIND (native `commands{twitter,reddit,parallel}` + `instructions` now emitted, `[≠]` narrowed to `scripts_dir`+conda literals, `to_dict` key order correct, U-023 structural asserts retained alongside new ones, signature unchanged) BUT the native guidance strings reference `POST /api/simulation/{simulation_id}/start` (id-in-PATH) while the authoritative start route (S-820 + `simulation.py:1451-1495`) is `POST /start` with id in the JSON BODY → guidance points at a nonexistent route. NATIVE-EXPRESSED guidance must be ACTIONABLE; this is a correctness defect, not a `[≠]`. Fix: change `commands.*` and `instructions` to `POST /api/simulation/start  body:{"simulation_id":"{id}","platform":"…"}`. Extension remains UNPROVEN until fixed (gates S-810). **U-026-c EXTENSION RE-VERIFY PARITY PASS 2026-06-19 (opus):** fix landed — `get_run_instructions` (`simulation_manager.rs:1689-1731`) now emits `endpoint = "POST /api/simulation/start"` and every `commands.{twitter,reddit,parallel}` + the `instructions` prose use the body-id form `body:{"simulation_id":"{id}","platform":"…"}`; NO id-in-path string remains in the function body (commands or prose). Cross-checked vs real start params `simulation.py:1495-1505` (simulation_id req, platform default parallel, max_rounds/enable_graph_memory_update/force opt) — instructions prose describes all five faithfully. Signature unchanged (additive); U-023 structural asserts (simulation_dir/config_file/substrate_note) retained. Regression guards MEANINGFUL & non-vacuous: `get_run_instructions_structural_fields` (`simulation_manager.rs:2186-2197`) asserts `cmd.contains("POST /api/simulation/start")` AND `!cmd.contains("/simulation/{id}/start")` AND the body `"simulation_id":"{id}"` — would FAIL on revert to id-in-path. 1309 pass / 6 ignored; both named tests green. NOTE (non-blocking, doc-only): stale id-in-path strings linger in DOC COMMENTS on `RunCommands`/`RunInstructions` (lines 722-724, 756, 777) and the module DECISION note — not in any executable/serialized output; flag to porter for cleanup.
...
- [x] S-804 · `unit:U-026` · `route` · `GET /entities/<graph_id>` · get_graph_entities · `simulation.py:49` → `api/simulation.rs::get_graph_entities`. **PARITY PASS 2026-06-19 (opus):** ZEP-guard empty→500 `api.zepApiKeyMissing` (`ApiError::client`, 2-key no-traceback); `entity_types` CSV = split/trim/drop-empty + `if s else None` (byte-match py:67); `enrich` = `.to_lowercase()=="true"` — ADVERSARIAL-verified `?enrich=1`→FALSE, `?enrich=yes`→FALSE, `?enrich=TRUE`→TRUE, absent→TRUE, NOT generic bool; success `{success,data:FilteredEntities::to_dict()}` key order `entities,entity_types,total_count,filtered_count`; unknown graph→500+traceback (matches py `filter_defined_entities` propagation). 6 route tests + 3 adversarial enrich tests green; 1327 lib pass.
- [x] S-805 · `unit:U-026` · `route` · `GET /entities/<graph_id>/<entity_uuid>` · get_entity_detail · `simulation.py:94` → `api/simulation.rs::get_entity_detail`. **PARITY PASS 2026-06-19 (opus):** ZEP-guard 500; reader `None`→404 `api.entityNotFound` with `t_args("id",uuid)` `{id}`-interpolation (en/zh.json:341, mirrors py `t(...,id=uuid)`); `Some`→`{success,data:EntityNode::to_dict()}` (7-key). **Flagged [≠] (defensible, recorded):** unknown graph_id → teri 500 vs Python 404 (Python's blanket `except→None→404` in `get_entity_with_context` is a non-portable Zep-always-constructable artifact; teri cannot build a reader over an absent graph — inexpressible substrate boundary; the PRIMARY entity-not-found 404 contract IS faithful; consistent w/ U-025(f)+DECISION-9). NOT a feature skip. 3 route tests + probe green.
- [x] S-806 · `unit:U-026` · `route` · `GET /entities/<graph_id>/by-type/<entity_type>` · get_entities_by_type · `simulation.py:127` → `api/simulation.rs::get_entities_by_type`. **PARITY PASS 2026-06-19 (opus):** ZEP-guard 500; same `enrich` rule as S-804 (shared parse); data key order EXACTLY `entity_type,count,entities` via explicit Map-insert; `count==entities.len()` invariant tested; no-match→count 0/entities []; routing edge `/by-type/X` (4 segs) resolves to this handler NOT get_entity_detail — non-vacuous shape assertion (`entity_type`+`count` present, `uuid` absent). 4 route tests + routing test green.
- [x] S-807 · `unit:U-026` · `route` · `POST /create` · create_simulation · `simulation.py:166` → `api/simulation.rs::create_simulation`. **PARITY PASS 2026-06-19 (opus):** all branches differential-verified vs `simulation.py:165-237` — empty-body→`{}` (`body: Option<Json>`), missing/empty project_id→400 `api.requireProjectId`, project-not-found→404 `api.projectNotFound` (id interpolated; locale strings byte-identical en+zh to MiroFish locales/*.json; `t_args` `{id}`-replace mirrors Python `value.replace('{id}',str(v))`), graph_id precedence (body non-empty wins else project.graph_id) + empty→400 `api.graphNotBuilt`, enable_twitter/reddit default-true (honored when false), success `{success,data:state.to_dict()}` (17-key to_dict order-identical, U-023-verified), 500→`ApiError::server` 3-key. 4 route tests green; 1309/1309 lib pass.
...
- [x] S-810 · `unit:U-026` · `route` · `GET /<simulation_id>` · get_simulation · `simulation.py:756` → `api/simulation.rs::get_simulation`. **PARITY PASS (RE-VERIFY) 2026-06-19 (opus) — fix landed, READY payload now actionable.** Verified-correct parts: not-found→404 `api.simulationNotFound` (id interpolated), success `{success,data:result}` with `result=state.to_dict()`, READY-gate condition `status==SimulationStatus::Ready` (non-READY omits run_instructions, READY includes it — both test-confirmed), `to_dict` outer key order `simulation_dir,config_file,commands{twitter,reddit,parallel},instructions,substrate_note` with `scripts_dir` justifiably absent. **DEFECT:** the embedded `run_instructions.commands.*` and `instructions` reference `POST /api/simulation/{simulation_id}/start` (id-in-PATH), but the authoritative start route (ledger S-820 + `simulation.py:1451-1495`) is `POST /start` with `simulation_id` carried in the JSON **body** — there is NO `/<id>/start` route in source and sub-cycle (g) will mount `POST /api/simulation/start`. The native guidance therefore points at a path that will not exist → non-actionable, fails the no-downgrade "concrete & actionable, real endpoint" bar. Stays `- [~]` until the porter fixes the guidance to the body-id start call. **RE-VERIFY 2026-06-19 (opus): FIXED.** READY branch (`api/simulation.rs:238-247`) inserts `run_instructions = get_run_instructions(...).to_dict()`; the embedded `commands.*` + `instructions` now reference EXACTLY `POST /api/simulation/start` with `simulation_id` in the JSON body (no id-in-path). Full nest path confirmed routable: `server.rs:199` nests `simulation_router` under `/simulation`, itself under `/api` (`server.rs:207`) → `/api/simulation`; sub-cycle (g) adds `POST /start` → full `POST /api/simulation/start`. Route regression guard `get_simulation_ready_has_run_instructions` (`api/simulation.rs:673-680`) asserts `cmd.contains("POST /api/simulation/start")` (non-vacuous; fails on revert). All previously-verified parts (404 shape, success envelope, READY-gate condition, to_dict key order, scripts_dir absent) unchanged. 1309 pass / 6 ignored.
- [x] S-811 · `unit:U-026` · `route` · `GET /list` · list_simulations · `simulation.py:789` → `api/simulation.rs::list_simulations`. **PARITY PASS 2026-06-19 (opus):** verified vs `simulation.py:788-814` — optional `?project_id` filter matches `SimulationManager::list_simulations(Option<&str>)` (None→all, Some→`project_id==pid`); body `{success,data:[to_dict...],count:len}` key order data-then-count; `count==data.len()` (test-confirmed). **`?limit` REFUTATION:** independently read `simulation.py:797` — Python reads ONLY `request.args.get('project_id')`, NO `?limit`. No narrowing. 3 route tests green; 1309/1309 lib pass.
- [ ] S-812 · `unit:U-026` · `route` · `GET /history` · get_simulation_history · `simulation.py:877`
- [x] S-813 · `unit:U-026` · `route` · `GET /<simulation_id>/profiles` · get_simulation_profiles · `simulation.py:991` · PARITY-VERIFIED-IN-TERI 2026-06-19 (e)
- [x] S-814 · `unit:U-026` · `route` · `GET /<simulation_id>/profiles/realtime` · get_simulation_profiles_realtime · `simulation.py:1029` · PARITY-VERIFIED-IN-TERI 2026-06-19 (e); CSV flexible/DictReader-padding round-2 fix
- [x] S-815 · `unit:U-026` · `route` · `GET /<simulation_id>/config/realtime` · get_simulation_config_realtime · `simulation.py:1139` · PARITY-VERIFIED-IN-TERI 2026-06-19 (e); empty-`{}`-summary round-2 fix
- [x] S-816 · `unit:U-026` · `route` · `GET /<simulation_id>/config` · get_simulation_config · `simulation.py:1259` · PARITY-VERIFIED-IN-TERI 2026-06-19 (e)
...
- [≠] S-818 · `unit:U-026` · `route` · `GET /script/<script_name>/download` · download_simulation_script · `simulation.py:1324` · **(e2) [≠] U026-SCRIPTDL** (parity PASS, challenge-survived): route + allowed-list validation ported VERBATIM (400 `unknownScript` byte-faithful incl Python `str(list)` repr of `allowed`); valid name → 404 `scriptFileNotFound` because teri ships no `run_*.py` (native in-process, DECISION-17 / S-601) — same 404 MiroFish returns when the file is absent. Owner-decision honored: route KEPT, never dropped, never 200-empty. `src/api/simulation.rs::download_script` + `ALLOWED_SCRIPTS`.
...
- [x] S-820 · `unit:U-026` · `route` · `POST /start` · start_simulation · `simulation.py:1452` · BOUNDARY-PARITY-VERIFIED-IN-TERI 2026-06-19 (g2); full validation+state-machine+check_simulation_prepared(auto-upgrade write)+cleanup_simulation_logs+graph_id resolution all ported+proven. **U-028 c3b-ii (2026-06-20, parity PASS):** GAP-U026-RUNINPUTS-BUILDER CLOSED for SINGLE-platform twitter/reddit — `build_run_inputs` assembles engine(config→ticks+activation+actions.jsonl producer)+pool+graph+llm, `/start` swaps the honest-500 for the real run → **200** with byte-faithful response (`run_state.to_dict()` RUNNING snapshot + `max_rounds_applied` only-when-Some + `graph_memory_update_enabled`/`force_restarted` always + `graph_id` only-when-memory; all 24 base keys + 4 add-on fields verified vs Python :1616-1622). ValueError→400 faithful via map_runner_err(Sim→400). `[≠]` eager-vs-lazy seam (U-022 RunInputs subprocess-boundary) recorded + unreachable past READY gate. **U-030 cycle B (2026-06-20, parity PASS → FLIP `[x]`, opus):** the last open piece — `[!] U028-PARALLEL-DUALSINK` — is CLOSED. The honest-500 `if platform=="parallel" {return Err(server)}` block at `/start` is DELETED (no orphan refs; old `start_simulation_prepared_reaches_gap_500` test removed; doc-comments on `build_run_inputs`/`start_simulation` updated). Parallel now flows through `build_run_inputs` → dual-logger producer (`PlatformLoggerSet::parallel(twitter,reddit)` over `load_agent_pool(sim_dir,"parallel")` unioned twitter+reddit pool, each agent carrying its own `social.platform` for per-file routing; activation policy `TimeActivationPolicy::from_config` unchanged, gates unioned pool by user_id) → `start_simulation` spawns run+monitor → **REAL 200** (NOT fabricated): same `run_state.to_dict()`+conditional-fields envelope as Python :1616-1627, platform-agnostic. `start_simulation_parallel_prepared_returns_200` seeds BOTH profile files, posts platform="parallel", asserts 200 + exact shape (`success:true`, no traceback, simulation_id, total_rounds==2, graph_memory_update_enabled:false, force_restarted:false, no max_rounds_applied, no graph_id). Single-platform path BYTE-IDENTICAL (just refactored into `make_logger` closure). E2E dual-gate proven by `parallel_producer_run_reaches_completed` (sim_mod boundary fan-out emits BOTH twitter/+reddit/actions.jsonl with full simulation_start→round×2→simulation_end stream even on empty pool; monitor reads twitter file as "twitter"/reddit as "reddit", sets `*_completed` strictly per-platform; `check_all_platforms_completed` requires BOTH when both files exist — load-bearing, NOT vacuous, proven by `check_completed_dual_requires_both`+`simulation_end_dual_one_platform_not_completed`). Cannot reach COMPLETED on one file. 1511 lib pass. ROUND-0 `initial_posts` stream content is S-877's contract (actions.jsonl record CONTENT during an LLM run), ORTHOGONAL to `/start`'s response/run-spawn contract — does NOT block S-820. Full S-820 contract now holds for ALL platforms → `- [x]`.
...
- [x] S-822 · `unit:U-026` · `route` · `GET /<simulation_id>/run-status` · get_run_status · `simulation.py:1706` · **(h) parity PASS**: None run_state → 200 with 8-key idle stub (runner_status:"idle", int-0 progress_percent, NOT 404, NOT full to_dict); else 200 run_state.to_dict(). `src/api/simulation.rs::run_status`.
- [x] S-823 · `unit:U-026` · `route` · `GET /<simulation_id>/run-status/detail` · get_run_status_detail · `simulation.py:1764` · **(h) parity FAIL→fix→PASS**: 5-key idle stub on None; else to_dict + all_actions/twitter_actions/reddit_actions/rounds_count/recent_actions (Python mutation order). Reads U-047 actions.jsonl tail. GATE CAUGHT empty-string `?platform=` downgrade (Python `not platform` falsy → reads BOTH + skips record filter; teri `Some("")` read neither) in SHARED get_all_actions/read_actions_from_file primitives — FIXED no_filter=`Some("")`-as-falsy + record-filter `.filter(|p|!p.is_empty())`; re-verified PASS, no over-correction (concrete filter unaffected), no shared-primitive regression. `[!] U026-h-ACTIONS-PRODUCER-PENDING` (tail empty until U-028/029/030; read path proven w/ real jsonl). `[~] U026-h-ROUNDS-LIVE` (rounds_count 0 on file-load both sides, faithful). `src/api/simulation.rs::run_status_detail`.
- [x] S-824 · `unit:U-026` · `route` · `GET /<simulation_id>/actions` · get_simulation_actions · `simulation.py:1865` · **(i) parity PASS**: {count,actions:[AgentAction.to_dict]} wrapper byte-exact; Flask type=int fallback (?limit=abc→100 not 400); agent_id/round_num None-default; empty-string ?platform= falsy via (h)-fixed get_all_actions. `src/api/simulation.rs::get_simulation_actions`.
- [x] S-825 · `unit:U-026` · `route` · `GET /<simulation_id>/timeline` · get_simulation_timeline · `simulation.py:1919` · **(i) parity FAIL→fix→PASS**: {rounds_count,timeline:[TimelineEntry.to_value]}. GATE CAUGHT dropped-field downgrade in SHARED primitive get_timeline/TimelineEntry (S-619, NOT actually verified by U-022(d)): Python entry = 9 keys ending first_action_time/last_action_time, teri emitted only 7 (fields never tracked — bare TODO comment). FIXED: added first/last to TimelineRound+TimelineEntry, get_timeline tracks per Python DESC-iteration (first-seen=NEWEST, last-overwritten=OLDEST, names intentionally inverted), to_value appends after action_types. Re-verified PASS (full 9-key order + DESC values). `src/api/simulation.rs::get_simulation_timeline`.
- [x] S-826 · `unit:U-026` · `route` · `GET /<simulation_id>/agent-stats` · get_agent_stats · `simulation.py:1959` · **(i) parity FAIL→fix→PASS**: {agents_count,stats:[AgentStats.to_value]}. GATE CAUGHT key-order downgrade in SHARED primitive AgentStats::to_value (S-620): Python order action_types BEFORE first/last timestamps, teri put action_types LAST (preserve_order → byte-observable). ALSO fixed latent get_agent_stats last_action_time never-updated bug (stuck at first-seen). Re-verified PASS (full 8-key order + DESC values). `src/api/simulation.rs::get_simulation_agent_stats`.
- [x] S-827 · `unit:U-026` · `route` · `GET /<simulation_id>/posts` · get_simulation_posts · `simulation.py:1988` · **(j) parity PASS r1**: missing-DB→200 4-key {platform,count:0,posts:[],message:dbNotExist} (faithful current behavior); populated SELECT * FROM post ORDER BY created_at DESC LIMIT/OFFSET + COUNT(*) behind #[cfg(feature=sqlite)] (rusqlite, row→dict by column name, OperationalError→empty), no-sqlite+DB-exists→honest-500 GAP-U026-SOCIALDB (no silent empty). sim_dir = oasis_simulation_data_dir/<id> (verifier traced all 3 Python paths resolve to same physical dir; no-create join). `[!] GAP-U026-SOCIALDB` OPEN (producer U-028/029/030 + sqlite feature). `src/api/simulation.rs::get_simulation_posts`.
- [x] S-828 · `unit:U-026` · `route` · `GET /<simulation_id>/comments` · get_simulation_comments · `simulation.py:2066` · **(j) parity PASS r1**: missing-DB→200 2-key {count:0,comments:[]}; populated SELECT * FROM comment [WHERE post_id=?] ORDER BY created_at DESC LIMIT/OFFSET behind sqlite feature (post_id filter verified), no-sqlite+DB-exists→honest-500. Same `[!] GAP-U026-SOCIALDB` family. `src/api/simulation.rs::get_simulation_comments`.
- [x] S-829 · `unit:U-026` · `route` · `POST /interview` · interview_agent · `simulation.py:2143` · **(k) parity PASS**: full validation (requireSimulationId/requireAgentId[agent_id 0 VALID]/requirePrompt/invalidInterviewPlatform) + env-alive gate→400 envNotRunning (check_env_alive Err→false, faithful to Python False). Success path optimize_prompt→interview_agent→shape_interview_result {success,agent_id,prompt[OPTIMIZED],result|error,timestamp} `[!] IPC-PRODUCER-PENDING`. `src/api/simulation.rs::interview_agent_route`.
- [x] S-830 · `unit:U-026` · `route` · `POST /interview/batch` · interview_agents_batch · `simulation.py:2272` · **(k) parity PASS**: + requireInterviews + per-item validation (interviewListMissingAgentId/Prompt/InvalidPlatform with 1-based index), env-gate→400. Shape {success,interviews_count:len,result|error,timestamp} `[!] IPC-PRODUCER-PENDING`. `interview_batch_route`. **>>> U026-k-TIMEOUT504 RESOLVED for S-829/830/831 (U-028 c1, 2026-06-19):** each interview route now maps `TeriError::Timeout` via `map_interview_err(e, <key>)` → **504** with its per-route i18n key (S-829 `api.interviewTimeout`, S-830 `api.batchInterviewTimeout`, S-831 `api.globalInterviewTimeout`) matching Python `:2256/2394/2497`. Timeout→504 + i18n-wrap unit-verified; live-env 504 producer-pending. `[≠] U028-c1-TIMEOUTMSG-NUMFMT` (cosmetic: int timeout renders `60.0秒` vs Python `60秒`; Duration collapses int/float).
- [x] S-831 · `unit:U-026` · `route` · `POST /interview/all` · interview_all_agents · `simulation.py:2410` · **(k) parity PASS**: requirePrompt + env-gate→400. interviews_count reconstructed via count_interview_agents (config agent_configs w/ agent_id, IDENTICAL predicate to runner's built-list) `[!] IPC-PRODUCER-PENDING`. `interview_all_route`.
- [x] S-832 · `unit:U-026` · `route` · `POST /interview/history` · get_interview_history · `simulation.py:2513` · **(k) parity PASS**: requireSimulationId; no-DB→200 {count:0,history:[]} (faithful both builds); #[cfg(feature=sqlite)] reads trace table action='interview' (verified populated); no-sqlite+DB-exists→honest-500 GAP-U026-SOCIALDB. `interview_history_route`. `[!] GAP-U026-SOCIALDB`.
- [x] (k-helper) · `unit:U-026` · `fn` · `optimize_interview_prompt` + INTERVIEW_PROMPT_PREFIX · `simulation.py:23,28` · **(k) parity PASS**: CJK prefix byte-identical (hex-verified), empty→empty, no-double-prefix. `src/api/simulation.rs`.
- [x] S-833 · `unit:U-026` · `route` · `POST /env-status` · get_env_status · `simulation.py:2585` · **(l) parity PASS**: pure file read, fully portable+tested TODAY (no producer dep). missing simulation_id→400 requireSimulationId; no env→200 5-key `{simulation_id, env_alive:false, twitter_available:false, reddit_available:false, message:envNotRunningShort}` (env_status_no_env_200_default_shape asserts exact 5-key shape+values); env_status.json `twitter_available:true` surfaces + env_alive INDEPENDENT of file flags (env_status_reads_env_status_json: env_alive:false yet twitter_available:true); `env_status.get("x",False)` → `.get("x").and_then(as_bool).unwrap_or(false)` faithful; read-error→default→false/false via `.unwrap_or_default()` (Python catches JSONDecodeError/OSError→default, 200 not 500) faithful. `src/api/simulation.rs::env_status_route`.
- [≠] S-834 · `unit:U-026` · `route` · `POST /close-env` · close_simulation_env · `simulation.py:2650` · **(l) parity PASS — divergences documented + adjudicated (re-verify 2026-06-19)**: validation + no-env 400 are FAITHFUL+tested (missing simulation_id→400 requireSimulationId; no registered run→`close_simulation_env` Err(`Simulation not found`)→map_runner_err(Sim→400) = Python ValueError→400; close_env_no_env_400 asserts 400 + no traceback). CJK literal `环境关闭命令已发送` byte-exact match. Success-path shaping `{success:status==completed, message, result|null, timestamp}`+`{success, data:result}`+status→Completed save is code-inspection-correct & producer-pending. `[≠] U026-l-ALREADYCLOSED` (Python check_env_alive-first→2-key `{success:True,message:"环境已经关闭"}` branch not produced — teri primitive always sends) is a DEFENSIBLE producer-pending `[≠]` (substrate uses in-process AtomicBool, branch only reachable with live IPC; flagged in route). **`[≠] U026-l-TIMEOUT` NOW DOCUMENTED + ADJUDICATED PASS** (the prior FAIL trigger): Python's primitive catches `TimeoutError`→**200** `{success:True,message:"环境关闭命令已发送（等待响应超时，环境可能正在关闭）"}`; teri's `send_command` elapsed→`Err(TeriError::Sim("…等待命令响应超时…"))`→route `.map_err(map_runner_err)`→**400/success:false**. Root cause: `TeriError` has NO `Timeout` variant (error.rs:4-55) — the 200-graceful-on-timeout shape is genuinely INEXPRESSIBLE without one; survives the `[≠]` bar. Path UNREACHABLE today (needs a live IPC run from U-028/029/030; close_simulation_env Err(no-run)→400 first). Same class as the already-accepted `[≠] U026-k-TIMEOUT504` (interview). Flagged in close_env_route header comment with byte-exact CJK literal + must-resolve-with-producer plan (add `TeriError::Timeout` → close-env 200-graceful / interview 504). `src/api/simulation.rs::close_env_route`. **>>> U026-l-TIMEOUT RESOLVED (U-028 c1, 2026-06-19):** added `TeriError::Timeout` (error.rs:21); IPC `send_command` elapsed→`Timeout` (simulation_ipc.rs:953); `close_env_route` now catches `Timeout`→graceful **200** with the byte-exact CJK literal `环境关闭命令已发送（等待响应超时，环境可能正在关闭）`. **No-downgrade gate CAUGHT a dropped side effect (B-1):** the early-return skipped the unconditional `status=COMPLETED + save` (Python `:2691-2696` runs it after the primitive swallows the timeout) — FIXED so the status-update runs for BOTH outcomes before the response branch. `[≠] U026-l-ALREADYCLOSED` unchanged. End-to-end live-env 200 remains producer-pending (cycle 3).
- [x] S-835 · `unit:U-026` · `route` · `GET /history` · get_simulation_history · `simulation.py:876` · **(m) parity PASS round-1 (2026-06-19)**: history list with project enrichment, portable TODAY (empty-run contract). `?limit` type=int default 20 (usize-parse, `[~] U026-m-NEGLIMIT` U-025 precedent); `list_simulations(None)[:limit]`; per-sim enrich appends 8 keys to the 17-key `to_dict` with `current_round` UPDATED IN PLACE at pos 12 (IndexMap re-insert keeps position = Python `dict[k]=v`) → 25-key order `…error, simulation_requirement, total_simulation_hours, runner_status, total_rounds, files, report_id, version, created_date` (history_key_order_preserved asserts full order). config block: requirement default ""/total_simulation_hours RAW value/recommended_rounds=`int(tsh*60/max(mpr||60,1))` truncate-toward-zero (history_recommended_rounds_truncates 5h/90min→3); run_state idle-fallback 0/"idle"/recommended (history_single_sim_defaults); files=project.files[:3]→`[{filename: f.get("filename","未知文件")}]` else [] (history_project_files_capped_and_defaulted: cap-3 + 未知文件 default); version "v1.0.2"; created_date=created_at[:10] char-safe. **report_id: porter chose `ReportManager::list_reports(Some(id),1).first()` over the loop_state-pointed `get_report_by_simulation` — VERIFIER CONFIRMED MORE FAITHFUL (not a downgrade)**: `get_report_by_simulation` (manager.rs:830) returns FIRST fs-iteration match; Python `_get_report_id_for_simulation` (simulation.py:817-873) sorts all matches by created_at DESC → NEWEST; `list_reports` `sort_by(b.created_at.cmp(a.created_at))` stable = Python stable Timsort reverse → byte-faithful newest, empty→null. VERIFIER PASS (9/9 symbols exercised at source; non-contractual edges: corrupt meta.json missing graph_id/requirement → Rust get_report `?`-rejects vs Python reads only sim_id/report_id [malformed-only], non-numeric minutes_per_round → Rust 60-default vs Python TypeError→500 [malformed-only] — both [~], not downgrades). `[!] U026-m-LIVEDATA` run-state/config/report read real disk; empty-run snapshot now, richer when U-028/029/030 land (same read path). +8 tests (1414→1422). `src/api/simulation.rs::get_simulation_history`.
- [x] S-836 · `unit:U-026` · `route` · `POST /prepare` · prepare_simulation · `simulation.py:359` · **(d) parity PASS (2026-06-19)**: async LLM-driven prepare; returns task_id immediately, background worker runs the 3-stage pipeline. FULL flow: id 400 requireSimulationId; get_simulation None→404 simulationNotFound (in-state Arc); force_regenerate default false; !force+check_simulation_prepared→200 ready/already_prepared:true/prepare_info (NO task_id — reuses verified check_simulation_prepared); get_project None→404 projectNotFound; empty simulation_requirement→400 projectMissingRequirement; document_text=get_extracted_text||""; entity_types(body)/use_llm_for_profiles(true)/parallel_profile_count(5); SYNC entity-count preview best-effort (resolve graph once → filter_defined_entities enrich=false → state.entities_count/entity_types, graph-resolve fail→warn+empty graph); create_task; status=PREPARING+save; spawn worker; 200 {simulation_id,task_id,status:preparing,message:prepareStarted,already_prepared:false,expected_entities_count,entity_types}. **VERIFIER CONFIRMED entity_types NOT conflated**: response uses preview-FOUND types (state.entity_types) while worker receives body-REQUESTED types — two distinct inputs (Python L484 vs L587). `[!] U026-d-GRAPHREQ` (prepare_simulation REQUIRES graph:&KnowledgeGraph, no live Zep — graph-resolve fail→empty graph→0 entities→FAILED terminal observable via /prepare/status, faithful to Python zero-entities→FAILED, NOT route 500). `src/api/simulation.rs::prepare_simulation_route`.
- [x] S-837 · `unit:U-026` · `route` · `POST /prepare/status` · get_prepare_status · `simulation.py:642` · **(d) parity PASS**: branch tree B1→B3b in Python order. B1 simulation_id+prepared→200 ready/100/already_prepared:true (runs FIRST, before task_id); B2a task_id absent+simulation_id present(not prepared)→200 not_started/0; B2b neither→400 requireTaskOrSimId; B3a task gone: simulation_id+(2nd)check_prepared→200 ready/100/task_id-echoed/taskCompletedPrepared else 404 taskNotFound (double check_simulation_prepared intentional/faithful); B3b task found→to_dict+already_prepared:false→200. VERIFIER CONFIRMED B1-precedes-task_id faithful. `src/api/simulation.rs::prepare_status_route`.
- [x] S-838 · `unit:U-026` · `fn` · `spawn_prepare_simulation` · run_prepare thread spawn · `simulation.py:491-612` · **(d) parity PASS — DECISION-U026-d-1-REVISED adjudicated FAITHFUL**: dedicated `std::thread::spawn` + current-thread tokio runtime (`Builder::new_current_thread().enable_all().block_on(with_locale(...))`), NOT tokio::spawn. WHY: prepare_simulation's OWN future is `!Send` (verifier confirmed at source — raw-ptr `*mut Option<&mut dyn FnMut>` simulation_manager.rs:1321 + `&mut dyn FnMut` held live across the `.await` at L1350 generate_profiles_from_entities); awaiting it in tokio::spawn would inherit !Send. std::thread+current-thread-runtime drives !Send futures on one thread → NO signature change to prepare_simulation/generate_config/raw-ptr trick (option a's cost rejected; git-confirmed blast radius=0 on U-023). MORE faithful to Python `threading.Thread(daemon=True)` (real OS thread). Preserves ALL observable behavior (immediate task_id, background progress, terminal complete/fail, locale re-applied, +runtime-build-fail→fail_task safety superset). `src/services/simulation_manager.rs::spawn_prepare_simulation`.
- [x] S-839 · `unit:U-026` · `fn` · `prepare_worker` · run_prepare body · `simulation.py:508-608` · **(d) parity PASS**: PROCESSING/0/startPreparingEnv → build Send progress closure (captures owned String task_id only) → prepare_simulation(Some(&mut cb)) → terminal: Ok(result_state)→complete_task(to_simple_dict) / Err→fail_task+reload-state-FAILED-save (best-effort, idempotent double-write matches Python two-layer). VERIFIER: zero-entities→Ok(state,status=failed)→complete_task (completed task whose RESULT reports failed, faithful Python L1261-1266+L593-597). pub(crate) for direct test-drive. `src/services/simulation_manager.rs::prepare_worker`.
- [x] S-840 · `unit:U-026` · `fn` · `prepare_progress_update` · inner progress_callback · `simulation.py:522-581` · **(d) parity PASS**: stage→overall-% mapping. stage_weights bands reading(0,20)/generating_profiles(20,70)/generating_config(70,90)/copying_scripts(90,100), unknown→(0,100); stage_index 1-based; total_stages=4; overall=int(start+(end-start)*progress/100) i64-truncate=Python int() for non-neg domain (tests assert 45 + truncated 11). stage_names i18n keys, unknown→raw stage. progress_detail 8 keys (current_stage/current_stage_name/stage_index/total_stages/stage_progress/current_item/total_items/item_description, current/total default 0). detailed_message total_items>0→"[idx/4] name: cur/tot - msg" else "[idx/4] name: msg". `[~] U026-d-STAGE4` (copying_scripts band kept for total_stages=4/index fidelity but never emitted by teri's pipeline — NOT a downgrade, stages 1-3 byte-identical, stage 4 fires in neither runtime). `src/services/simulation_manager.rs::prepare_progress_update`. +14 tests (1422→1436).
- [x] S-841 · `unit:U-027` · `route` · `GET /:report_id` · get_report · `report.py:277` · **(a) parity PASS (2026-06-19)**: get_report None→404 reportNotFound{id} / Some→200 {success,data:to_dict}. `src/api/report.rs::get_report_route`.
- [x] S-842 · `unit:U-027` · `route` · `GET /by-simulation/:sim` · get_report_by_simulation · `report.py:319` · **(a) parity PASS**: None→404 {success:false,error:noReportForSim,has_report:false} (has_report TOP-LEVEL via client_with) / Some→200 {success,data,has_report:true}. `src/api/report.rs::get_report_by_sim_route`.
- [x] S-843 · `unit:U-027` · `route` · `GET /list` · list_reports · `report.py:358` · **(a) parity PASS**: ?simulation_id optional filter + ?limit type=int default 50 (usize-parse, `[≠] U025-LIMIT-PARSE` negative-slice non-contractual established precedent) → {success,data:[to_dict],count}. `src/api/report.rs::list_reports_route`.
- [x] S-844 · `unit:U-027` · `route` · `DELETE /:report_id` · delete_report · `report.py:444` · **(a) parity PASS**: delete_report false→404 reportNotFound / true→200 {success,message:reportDeleted} (message NOT data). Shares /:report_id path with S-841 via get(...).delete(...). `src/api/report.rs::delete_report_route`.
...
- [x] S-852a · `unit:U-027` · `route` · `GET /:report_id/download` · download_report · `report.py:398` · **(c) parity PASS (2026-06-19)**: get_report None → 404 reportNotFound{id}; else serves on-disk full_report.md (GAP-A) `.unwrap_or(markdown_content)` as `text/markdown` attachment, CD=`attachment; filename="{id}.md"`, body==markdown bytes. Both source branches (md_path / temp from markdown_content) yield SAME bytes; teri parity-faithful. CT non-contractual for attachment. Returns `Response` not `Json`. `src/api/report.rs::download_report_route`.
- [x] S-852b · `unit:U-027` · `route` · `GET /:report_id/sections` · get_report_sections · `report.py:610` · **(c) parity PASS**: get_generated_sections(id) + is_complete=report&&COMPLETED → {success,data:{report_id, sections, total_sections:len, is_complete}}. 4-key data MATCH; Generating→false, Completed→true, no-report→false all verified. `src/api/report.rs::get_sections_route`.
- [≠] S-852c · `unit:U-027` · `route` · `GET /:report_id/section/:idx` · get_single_section · `report.py:661` · **(c) parity PASS, `[≠] U027-c-SECTIONIDX-404BODY`**: valid+present → 200 {filename:section_{NN:02}.md, section_index, content}; valid+missing → 404 sectionNotFound{index:02d} ("05" for idx 5). Flask `<int:>` non-match→Werkzeug HTML 404; axum has NO type-route-converter (context7 verbatim) so typed Path<usize> would 400 — porter corrected via String-capture+manual-parse → faithful 404 STATUS; body shape (teri JSON vs Flask default HTML page, no JSON 404 errorhandler in __init__.py) is non-contractual on a malformed URL the frontend never sends (constructs /section/{int}). CHALLENGED→SURVIVES: not inexpressible, genuinely non-contractual, NOT a portable-feature-skip. `src/api/report.rs::get_single_section_route`.
- [x] S-852d · `unit:U-027` · `fn` · `ReportManager.read_report_markdown` (GAP-A pub wrapper) · `report.py:414-433` · **(c) parity PASS**: thin wrapper over private `get_report_markdown_path` → Some(on-disk full_report.md) | None(fallback). `git diff --stat` = +31/-0 → private helper byte-unchanged. `src/report/manager.rs:534`.
- [x] S-852e · `unit:U-027` · `method` · `ReportManager.get_single_section` (GAP-B pub wrapper) · `report.py:676-694` · **(c) parity PASS**: thin wrapper over private `get_section_path` → Some((filename `section_{NN:02}.md`, content)) | None(→404). Private helper byte-unchanged (+31/-0 diff). `src/report/manager.rs:544`.
- [x] S-852f · `unit:U-027` · `route` · `POST /tools/search` · search_graph_tool · `report.py:935` · **(d) parity PASS (2026-06-19)**: body{graph_id,query,limit=10}; `not graph_id or not query`→400 requireGraphIdAndQuery (validation BEFORE ZEP guard, falsy absent/empty equivalence verified); else `load_entity_reader_graph`(REUSED, ZEP guard+task→graph)→`ReportTools::new(&graph,&llm)`→`search_graph(graph_id,query,limit,Some("edges"))` [scope omitted in source→Python default "edges"]→`SearchResult::to_dict` 5-key {facts,edges,nodes,query,total_count}, `{success,data}`. Send-safe (only await yields owned graph; search sync after). Flags: `[≠]U026-ZEPKEY`(KEPT, 500 zepApiKeyMissing), `[≠]U026-R2-ABSENTGRAPH`(unresolvable graph_id→500 vs Python empty — substrate-forced narrowing, primary resolved-graph→to_dict faithful, same as entities-route), `[!]U027-GRAPHREQ`(resolved-empty→empty result faithful), `[≠]U025-TRACEBACK`. VERIFIER tried-to-refute on 7 axes, all hold. `src/api/report.rs::tools_search_route`.
- [x] S-852g · `unit:U-027` · `route` · `POST /tools/statistics` · get_graph_statistics_tool · `report.py:983` · **(d) parity PASS**: body{graph_id}; `not graph_id`→400 requireGraphId; else `load_entity_reader_graph`→`ReportTools::get_graph_statistics(graph_id)`→5-key {graph_id,total_nodes,total_edges,entity_types,relation_types} as `data` DIRECTLY (Python result is already a plain dict, NOT double-wrapped via to_dict). graph_id echoed; preserve_order key-order byte-faithful. Same ZEP/absent-graph flags as S-852f. `src/api/report.rs::tools_statistics_route`. **(d) also promoted `load_entity_reader_graph` sim:205 → `pub(crate)` — REUSE-not-duplicate, one graph-resolution path / one ZEP guard, blast-radius=visibility only.**
- [x] S-852h · `unit:U-027` · `route` · `POST /chat` · chat_with_report_agent · `report.py:472` · **(e) parity PASS r1 (2026-06-19)**: full resolution chain — `not simulation_id`→400 requireSimulationId (before) `not message`→400 requireMessage; `sim_manager.get_simulation`None→404 simulationNotFound{id}; `ProjectManager::from_config.get_project(sim.project_id)`None→404 projectNotFound{**project_id**}; graph_id=`state.graph_id or project.graph_id` (sim-nonempty wins, else project.graph_id.unwrap_or_default), empty→400 missingGraphId; requirement=`project.simulation_requirement.unwrap_or_default()` (=Python `or ""`); `load_entity_reader_graph`(ZEP guard)→`ReportTools::new`+`build_llm`+`report_manager`→`ReportAgent::new_react(graph_id,sim_id,requirement)`→`agent.chat(&tools,&llm,&manager,&message,&history).await`→`ChatResponse::to_dict` 3-key {response,tool_calls,sources}. **PLAIN async fn — NO RefCell-across-await (unlike generate_report) → ordinary Send axum handler, NO OS-thread** (compiled as post(chat_route), confirmed Send). `[!]U027-e-LLM-GATED`: 200 success drives live LLM (ReACT), not unit-tested here — but ENTIRE pre-LLM contract tested (2×400, 2×404, graph_id fallback BOTH branches, ZEP-500 doubles as full-resolution proof); chat substrate already U-024-verified w/ mock adapters; success wiring correct by inspection. VERIFIER refuted 9 axes ALL hold; confirmed NO double-windowing (handler passes full array, chat does [-10:] internally). `src/api/report.rs::chat_route`.
- [x] S-852i · `unit:U-027` · `fn` · `parse_chat_history` helper · `report_agent.py:1814-1815` · **(e) parity PASS**: parses `chat_history` JSON array→Vec<ChatMessage>; `[~]U027-e-CHATROLE-NARROW` Python appends role-dicts VERBATIM (arbitrary role strings) but teri ChatRole closed enum → system/assistant mapped, ELSE(user/unknown/absent)→user; frontend contract (report.py:484-485) only sends user/assistant → non-contractual narrowing NOT downgrade. Non-array/absent→empty. Unit-tested (5-entry role-map + non-array). `src/api/report.rs::parse_chat_history`.
- [x] S-852j · `unit:U-027` · `route` · `POST /generate` · generate_report · `report.py:25` · **(f) parity PASS r1 (2026-06-19)**: async keystone. Validate sim_id(400 requireSimulationId)→get_simulation None→404 simulationNotFound{id}→`!force_regenerate && existing COMPLETED`→200 {report_id,status:completed,reportAlreadyExists,already_generated:true}→project None→404 projectNotFound{project_id}→graph_id=`state.graph_id or project.graph_id` empty→400 **missingGraphIdEnsure** (distinct key from chat's missingGraphId, both source-confirmed)→requirement REQUIRED empty→400 missingSimRequirement→mint report_id=`report_{uuid_hex[:12]}`→`load_entity_reader_graph`(ZEP guard, `[≠]U027-f-GRAPHRESOLVE-EAGER`)+build_llm→`TaskManager::create_task("report_generate",{sim_id,graph_id,report_id})`→`spawn_report_generation`→immediate 200 {simulation_id,report_id,task_id,status:generating,reportGenerateStarted,already_generated:false}. `src/api/report.rs::generate_report_route`.
- [x] S-852k · `unit:U-027` · `route` · `POST /generate/status` · get_generate_status · `report.py:203` · **(f) parity PASS**: `if simulation_id truthy`(!is_empty)+existing COMPLETED→200 {report_id,status:completed,progress:100,reportGenerated,already_completed:true}; `not task_id`→400 requireTaskOrSimId; `get_task` None→404 taskNotFound{id}; else 200 `task.to_dict()`. No reachable 500 (get_report_by_simulation/get_task infallible) → source no-traceback outer-except moot. `src/api/report.rs::generate_status_route`.
- [x] S-852l · `unit:U-027` · `fn` · `spawn_report_generation` (OS-thread spawn) · `report.py:179-180` · **(f) parity PASS, DECISION (i)**: mirrors `spawn_prepare_simulation` VERBATIM — `std::thread::spawn` + `Builder::new_current_thread()` drives the `!Send` generate_report future (RefCell<&mut dyn ReportSink> across await, mod:1672); locale capture+with_locale re-apply; rt-build-fail→fail_task. Faithful port of Python `threading.Thread(daemon=True)`. tokio::spawn would NOT compile (!Send). Blast-radius 0 (no generate_report signature change). `src/api/report.rs::spawn_report_generation`.
- [x] S-852m · `unit:U-027` · `fn` · `report_generate_worker` (pub(crate)) · `report.py:128-176` · **(f) parity PASS**: update_task(PROCESSING,0,initReportAgent)→new_react→ReportTools::new+ReportManager::new→`agent.generate_report(&tools,&llm,&manager,&mut sink,Some(report_id)).await`→save_report(Err→fail_task = Python outer except→fail_task)→COMPLETED→complete_task({report_id,simulation_id,status:completed}) else fail_task(`report.error or reportGenerateFailed` empty→default). Worker-test drives to terminal state (generate_report ALWAYS returns terminal Report — Completed on Ok/Failed on Err, never Pending; graceful LLM-fallback completes even w/o LLM — substrate real behavior, not test-mask). `src/api/report.rs::report_generate_worker`.
- [x] S-852n · `unit:U-027` · `type` · `TaskUpdateSink` (ReportSink impl) · `report.py:146-151` · **(f) parity PASS**: progress_callback analog — `event(ev)`→`TaskManager::update_task(progress=ev.progress as i64, message=format!("[{}] {}", ev.stage.to_status_str(), ev.message))`; holds only String → Send (satisfies ReportSink:Send held across awaits). `src/api/report.rs::TaskUpdateSink`. **>>> U-027 COMPLETE: all 6 sub-cycles a-f landed+parity-verified (S-841-852n). `[!]U027-f-LLM-GATED`: worker LLM round-trip producer-gated, full pre-spawn contract tested. `[≠]U027-f-GRAPHRESOLVE-EAGER` legit (consistent w/ prepare precedent). create_app S-024 now has all 3 blueprints (U-025/026/027) nested — only register_cleanup (U-023/U-049) remains before S-024 flips.**
...
<!-- U-028 CYCLE 1 (2026-06-19, map-onto-substrate, parity PASS): substrate-wiring deliverables that
     close the `GAP-U026-RUNINPUTS-BUILDER` engine half + clear two `[≠]`s. NO platform S-row (S-853..)
     flips yet — each is a Python OASIS-script symbol whose FULL contract (run loop, env.step, IPC
     subprocess) is producer-pending in cycles 2-3; cycle 1 lands the deterministic builders only.
  (a) `SimConfig::from_simulation_config(config, max_rounds, parallelism)` (sim/mod.rs) — the
      config→engine tick mapping: max_ticks = int((total_simulation_hours*60)/minutes_per_round)
      truncated, then min(.,max_rounds>0). FORMULA-IDENTICAL to the landed status-field math at
      simulation_runner.rs:1091-1118 (ONE truncation impl). Pure table test (5 cases incl 10h/7min=85,
      max_rounds {0,-5}→no-truncate, zero-cadence→0, absent-key→72/30 defaults) differential vs Python.
      Realizes the `engine`-field side of u026-g §0. The `pool` field + activation are cycles 2-3.
  (b) `TeriError::Timeout` variant + faithful 504/graceful-200 routing — resolves `U026-k-TIMEOUT504`
      (S-829/830/831) + `U026-l-TIMEOUT` (S-834); see those rows. Gate caught + fixed close-env B-1
