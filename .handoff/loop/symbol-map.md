...
- [ ] S-023 · `unit:U-002` · `fn` · `main` · Flask entry-point; validates config, reads FLASK_HOST/PORT, runs threaded=True · `run.py:25`

---

## U-003 — `backend/app/__init__.py:create_app`

- [ ] S-024 · `unit:U-003` · `fn` · `create_app` · Flask app factory; registers blueprints, middleware, signal handlers · `__init__.py:19`
- [ ] S-025 · `unit:U-003` · `route` · `GET /health` · returns `{"status":"healthy"}` · `__init__.py`
...
- [ ] S-189 · `unit:U-015` · `method` · `GraphBuilderService.build_graph_async` · spawns background thread → task_id · `graph_builder.py:54` · **RE-AUDIT 2026-06-14: PENDING-DEPENDENCY→U-012 (reclassified from `[≠]`).** Real async-task-with-progress FEATURE (`self.task_manager.create_task` → task_id, progress). NOT a permanent divergence — belongs to TaskManager (U-012, S-138..S-167, all `- [ ]`). Port when U-012 lands.
...
- [ ] S-192 · `unit:U-015` · `method` · `GraphBuilderService.set_ontology` · dynamic Pydantic EntityModel/EdgeModel subclasses · `graph_builder.py:205` · **RE-AUDIT 2026-06-14: PENDING-DEPENDENCY→U-014 (reclassified from `[≠]`).** Dynamic ontology (entity/edge models from an ontology dict) IS in scope (OQ-5/GAP-3: EntityKind::Custom + ontology generator). NOT a permanent divergence — belongs to OntologyGenerator (U-014, S-172..S-176, all `- [ ]`). Port when U-014 lands.
...
- [~] S-356 · `unit:U-018` · `method` · `OasisProfileGenerator._build_entity_context` · **PORTED 2026-06-14 (ITERATE cycle).** → `PersonaGenerator::build_entity_context(graph: &KnowledgeGraph, entity: &Entity) -> String` (src/agent/mod.rs) + `generate_social` extended with `graph_ctx: Option<(&KnowledgeGraph, &Entity)>` param. In-process parts 1+3 ported: entity attributes section (name+kind), related entities section (neighbor name+kind from `get_neighbors`). Zep-search half (part 4) stays `[≠]` (S-355). Flat fallback when entity has no neighbors. Tests: enrichment-present when graph_ctx provided (assert prompt contains neighbor name), no-enrichment when None, graceful with no-neighbor entity. Parity unproven (verifier pending).
