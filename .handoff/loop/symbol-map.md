...
- [ ] S-605 · `unit:U-022` · `field` · `SimulationRunner._monitor_threads` · `simulation_runner.py:223` · **[→sub-cycle c]** `RunHandle::monitor: Option<JoinHandle<()>>` field placed in (b) (`None`), monitor task spawned in (c); stop/cleanup abort it if `Some`.
