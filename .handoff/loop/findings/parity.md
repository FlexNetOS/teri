...
## Cycle 64 — U-024 ReportAgent ReACT tool machinery (2026-06-20) — 14 symbols, REUSE-Y verify-only

**Source X:** `MiroFish/backend/app/services/report_agent.py`
**Dest Y:** `src/services/zep_tools.rs` (tool machinery) + `src/report/mod.rs` (ReportAgent struct/loop)
**Method:** differential read X↔Y, byte-diff of const strings, Config-default cross-check, full test-suite run. NO Rust changes (ledger-only).

**Test baseline (green, unchanged):**
- `cargo test -p teri --lib zep_tools::` → **77 passed**, 0 failed.
- `cargo test -p teri --lib report::` → **210 passed**, 0 failed.

### Per-symbol verdict

- **S-734 `TOOL_DESC_INSIGHT_FORGE`** → `zep_tools.rs:1365`. **`[x]`** — byte-identical, 217 chars (Python extracted vs Rust unescaped, `IDENTICAL`). Covered by `test_tool_desc_insight_forge_verbatim`.
- **S-735 `TOOL_DESC_PANORAMA_SEARCH`** → `zep_tools.rs:1384`. **`[x]`** — byte-identical, 212 chars. `test_tool_desc_panorama_search_verbatim`.
- **S-736 `TOOL_DESC_QUICK_SEARCH`** → `zep_tools.rs:1402`. **`[x]`** — byte-identical, 104 chars. `test_tool_desc_quick_search_verbatim`.
- **S-737 `TOOL_DESC_INTERVIEW_AGENTS`** → `zep_tools.rs:1415`. **`[x]`** — byte-identical, 505 chars INCLUDING the embedded escaped quotes around `"采访实录"` (Rust `\"…\"` unescapes to Python's `"…"`). `test_tool_desc_interview_agents_verbatim`.
- **S-750 `ReportAgent` type** → `report/mod.rs:635` `pub struct ReportAgent`. **`[x]`** — carries graph_id/simulation_id/simulation_requirement (the three `__init__` context fields) + report_logger/console_logger. `test_new_react_fields`, `test_new_returns_empty_fields`.
- **S-751 `MAX_TOOL_CALLS_PER_SECTION`** → `report/mod.rs:1031` `const … = 5`. **`[x]`** — REFUTATION attempted (narrowing check): task note says "from Config", but `report_agent.py:876` HARDCODES `MAX_TOOL_CALLS_PER_SECTION = 5` as a class attr, never reassigned, and report_agent never reads `Config.REPORT_AGENT_MAX_TOOL_CALLS` (grep: only `self.MAX_TOOL_CALLS_PER_SECTION` references exist; no `= Config.…` override). So Python is ALSO hardcoded-5 with no runtime override path → teri's hardcoded 5 is faithful, NOT a `[≠]` narrowing. Used at report/mod.rs:1211/1282 exactly as Python uses it at report_agent.py:1407/1453.
- **S-752 `MAX_REFLECTION_ROUNDS`** → teri: NOT DEFINED. **`[≠]` (non-contractual / dead-source-symbol)**. REFUTATION: is this a portable-feature skip? NO. `report_agent.py:879` defines `MAX_REFLECTION_ROUNDS = 3` but it is **dead code** — grep across report_agent.py returns the definition line ONLY; no `self.MAX_REFLECTION_ROUNDS` read anywhere, no reflection loop consumes it. It produces ZERO observable behavior in the source. Omitting an unreferenced source constant is not a downgrade (there is no distinct observable output — no serialization, no CLI flag, no render path tied to it). Survives the `[≠]` bar as genuinely non-contractual/unobservable. (Note: Config.REPORT_AGENT_MAX_REFLECTION_ROUNDS=2 is a SEPARATE config key, also unread by report_agent — irrelevant to this symbol.)
- **S-754 `__init__`** → `report/mod.rs:677` `new_react`. **`[x]`** — sets the 3 context fields + logger fields (Python `__init__` sets graph_id/simulation_id/simulation_requirement + self.tools + report_logger/console_logger=None; teri reifies tools as the `ReportTool` enum + inline table rather than a per-instance dict — equivalent observable mapping). agentInitDone log mirrored. `test_new_react_fields`.
- **S-755 `_define_tools`** → reified as the `ReportTool` enum (`zep_tools.rs:1452`) + the tool table in `get_tools_description` (`zep_tools.rs:1671`). **`[x]`** — the name→description→params mapping (the only observable of `_define_tools`, consumed solely by `_get_tools_description`) is byte-faithful: insertion order insight_forge→panorama_search→quick_search→interview_agents, params keys/values match report_agent.py:921-953. `test_get_tools_description_order`, `_contains_all_tools`, `_contains_params`.
- **S-756 `_execute_tool`** → `zep_tools.rs:1733` `execute` / `1779` `execute_inner` / `1921` `execute_by_name`. **`[x]` for DISPATCH STRUCTURE** (interview_agents LEAF remains an honest `[!]`). REFUTATION (high-risk): routing of all 4 primary names + 5 back-compat names (search_graph→quick_search, get_simulation_context→insight_forge, get_graph_statistics/get_entity_summary/get_entities_by_type direct) is faithful to the if/elif chain at report_agent.py:971-1058; unknown-tool returns the byte-identical `"未知工具: {name}。请使用以下工具之一: insight_forge, panorama_search, quick_search"` (report_agent.py:1058); str→int/bool param coercions (limit, max_agents.min(10), include_expired) mirror report_agent.py:985-1014; outer try/except → `"工具执行失败: {e}"` mirrored. **ACTUAL leaf state of interview_agents** (`zep_tools.rs:1259`): STILL honest-errors (`Err("interview_agents pending simulation IPC integration (sub-cycle e, U-022)")`) — the leaf was NOT re-wired to `SimulationRunner::interview_agents_batch` (S-631, which IS now `[x]` in U-022). Because the gap is HONEST (returns Err → swallowed to `"工具执行失败: …"`, never a silent wrong answer), the DISPATCH is faithful and the unit can carry it; the interview LEAF stays a tracked `[!]` (covered by `test_interview_agents_returns_honest_error`, `test_execute_by_name_interview_agents_returns_error_text`). Dispatch tests: `test_execute_by_name_*` (quick_search/panorama/insight/unknown/all 5 redirects), `test_execute_max_agents_capped_at_10`, `test_execute_interview_agents_topic_fallback_to_query`, `test_execute_quick_search_bad_limit_returns_failure_text`.
- **S-757 `VALID_TOOL_NAMES`** → `zep_tools.rs:1531` `[&str; 4]`. **`[x]`** — exact set `{insight_forge, panorama_search, quick_search, interview_agents}` (report_agent.py:1065). Gates tier-2/tier-3 parse exactly as Python. Covered by tier2/tier3 valid-vs-invalid rejection tests.
- **S-758 `_parse_tool_calls`** → `zep_tools.rs:1546` `parse_tool_calls`. **`[x]`** (highest-risk, deeply refuted). 3-tier priority byte-faithful to report_agent.py:1067-1112: tier1 XML `(?s)<tool_call>\s*(\{.*?\})\s*</tool_call>` raw (NO gate, NO normalize — matching Python's raw `json.loads` + downstream `.get`); tier1 short-circuits if non-empty; tier2 bare whole-response JSON `{…}` gated+normalized; tier3 trailing `(?s)(\{"(?:name|tool)"\s*:.*?\})\s*$` gated+normalized. The one `[≠]` (tier-1 name-less object skipped gracefully vs Python KeyError-crash downstream) is a Python DEFECT not preserved — observable only on malformed LLM output, well-formed path byte-faithful; this is a strict robustness superset, survives `[≠]` bar. 15 tests: tier1 single/multiple/dotall/bad-json-skip/params-not-normalized/tool-key-skip/raw-params, tier2 bare/invalid-rejected/text-before/normalize-tool/normalize-params, tier3 name-key/tool-key/unknown-rejected, empty/plain-text.
- **S-759 `_is_valid_tool_call`** → `zep_tools.rs:1618` `is_valid_tool_call` + `normalize_tool_call_keys`. **`[x]`** — accepts name|tool key, gates on VALID_TOOL_NAMES, in-place renames tool→name and params→parameters (only when parameters absent) exactly as report_agent.py:1114-1125. Covered via tier2/tier3 valid/invalid + `test_parse_tool_calls_tier2_normalizes_tool_to_name`, `_normalizes_params_to_parameters`.
...
## 2026-06-21 · Cycle 72 (FINAL) · Runtime-construct contracts U-045/046/049/050 + SWEEP-3 OASIS

**Gate:** rust-port-parity-verifier (differential, cross-boundary; adversarial — tried to REFUTE "teri realizes the contract"). Both sides cited file:line.

### U-045 SUBPROCESS_ISOLATION_CONTRACT → **REALIZED**
- X: separate OS process group; `_terminate_process` (`simulation_runner.py:721`) `os.killpg(pgid, SIGTERM)` (L766) → `process.wait(timeout)` → `os.killpg(pgid, SIGKILL)` (L773) on TimeoutExpired. `stop` → default `timeout=10` (L793/L721); `cleanup_all` → explicit `timeout=5` (L1224).
- teri: in-process tokio model (no subprocess). `terminate_handle` (`simulation_runner.rs:2124`): set cooperative `shutdown` AtomicBool (SIGTERM analog) → `tokio::time::timeout(grace, &mut handle.task)` → on Elapsed `handle.task.abort()` (SIGKILL analog) + reap. `STOP_GRACE=10s` / `CLEANUP_GRACE=5s` (rs:878/886) match Python per-caller timeouts EXACTLY. Isolation = each run is a separate `tokio::spawn`ed task with its own owned context. `[≠]` non-zero-exit-code→FAILED branch (no OS exit code in-process) survives the bar — run-failure observable carried by sim-task error logging + `Failed` transitions. Grace-window test `terminate_grace_windows_match_python_defaults` (rs:4300).

### U-046 IPC_POLLING_CONTRACT → **REALIZED**
- X: filesystem IPC — `ipc_commands/`→scan→`ipc_responses/`, 0.5s poll (`run_twitter_simulation.py:684` `wait_for(..., timeout=0.5)`), `env_status.json` written by `update_status` (L162) with ONLY `{status,timestamp}` (never twitter_available/reddit_available).
- teri: in-process mpsc<IpcEnvelope> + per-cmd oneshot reply (`simulation_ipc.rs:756+`); liveness = `Arc<AtomicBool>`. Protocol types/shapes/ordering/timeouts PORTED (CommandType/CommandStatus snake_case .value, IPCCommand/Response to_dict key-order + null-not-omitted, FIFO, real elapsed `tokio::time::timeout` 60/120/30s). **Adversarial check:** `get_env_status_detail` (rs:5397) still reads `env_status.json` which teri never writes → always returns defaults. REFUTED as a gap: the `/env-status` route (rs:3195 vs X `simulation.py:2585`) surfaces `env_alive` (← AtomicBool, the live analog of `status=="alive"`) + `twitter_available`/`reddit_available` (**ALWAYS False on BOTH sides** — X never writes those keys) + `message` (keyed off env_alive). The `status` field of env_status.json is surfaced NOWHERE observable. File transport + 0.5s poll + JSONDecodeError-retry are non-contractual `[≠]` (channel wakes immediately, values moved whole).

### U-049 BACKGROUND_THREAD_CONTRACT → **REALIZED**
- X monitor daemon thread (`simulation_runner.py:463-469`, daemon=True) → teri `spawn_monitor_task` (rs:2211) `JoinHandle`, aborted in `terminate_handle` (the "dies with run" observable; daemon-flag `[≠]` legit).
- X shared state under `threading.Lock` → teri `Arc<tokio::sync::Mutex<SimulationRunState>>` (rs:914) + cache `Mutex<HashMap>` (`simulation_manager.rs:854`).
- X prepare bg thread (report.py / prepare route `run_prepare`) → teri `spawn_prepare_simulation` (`simulation_manager.rs:1836`) dedicated `std::thread::spawn` (future `!Send`, documented DECISION-U026-d-1-REVISED); report bg thread (X `report.py:179` `threading.Thread(daemon=True)`) → teri `report.rs:808` `std::thread::spawn` + current-thread runtime.
- X `ThreadPoolExecutor(max_workers=parallel_count)` profile gen (`oasis_profile_generator.py:959`) → teri `futures::stream::iter(...).buffer_unordered(parallel_count.max(1))` (`oasis_profile_export.rs:522-566`) — bounded concurrency, order-preserving. No lost concurrency path.

### U-050 SIGNAL_CONTRACT → **GAP** (no-downgrade violation)
- X: `register_cleanup` (`simulation_runner.py:1319-1357`) installs `cleanup_handler` for **SIGTERM + SIGINT + SIGHUP** (has_sighup guard, L1351), each calling `cleanup_all_simulations()` then chaining the original handler; atexit backup; Flask SIGTERM propagates kill to child pgid.
- teri: `server.rs:294-296` installs **ONLY `tokio::signal::ctrl_c()` = SIGINT**. The graceful-shutdown closure does NOT call `cleanup_all` (TODO `PENDING U-023/U-049: call SimulationRunner::cleanup() here`, rs:297). `cleanup_all` (`simulation_runner.rs:1365`) EXISTS and is correct (5s grace) but is invoked from **tests only** — never any signal path. teri's own comment (rs:847-848): "ships `cleanup_all` as the callable U-049 will invoke; **it does NOT install signal handlers.**"
- **Lost behaviors (3-of-3 degraded):** (1) **SIGTERM unhandled** — `tokio::signal::ctrl_c()` catches SIGINT only; under `kill`/container/systemd shutdown (dominant production path) teri is hard-killed at OS default → running sims NOT gracefully cancelled, NOT recorded STOPPED, no `cleanup_all`. (2) **SIGHUP unhandled** — X handles it; teri does not. (3) even the handled SIGINT does NOT propagate cancellation to running simulations (no `cleanup_all` call). Ledger unit "U-049 Signal/graceful shutdown" (parity-ledger.md:175) is still `- [ ]`. **FIX REQUIRED:** install `tokio::signal::unix::SignalKind::{terminate,hangup}` + SIGINT handlers, all selecting into the graceful-shutdown future that calls `SimulationRunner::cleanup_all().await` before axum stops. Substrate (`cleanup_all`) is landed — only the handler wiring is missing; NOT inexpressible → a portable feature skipped → GAP, route to porter.

### SWEEP-3 OASIS_CONTRACT → **REALIZED** (with a legitimate substrate `[≠]`)
- ActionType variant set: X recordable set = `ACTION_TYPE_MAP` (`run_parallel_simulation.py:614-629`) = 14 social types + INTERVIEW. **REFRESH is in `FILTERED_ACTIONS`** (L611, refresh/sign_up filtered — never recorded). teri `SocialAction::oasis_action_type` (`sim/mod.rs:100-118`) covers ALL 14 recordable: CREATE_POST, LIKE_POST, LIKE_COMMENT, DISLIKE_POST, DISLIKE_COMMENT, REPOST, QUOTE_POST, FOLLOW, CREATE_COMMENT, SEARCH_POSTS, SEARCH_USER, MUTE, TREND, DO_NOTHING. REFRESH correctly omitted as `[≠]` (filtered). Twitter-6 ⊆ / Reddit-13 ⊆ this set (the 6+13 with REFRESH filtered = 14 distinct recordable). INTERVIEW handled via IPC ManualAction (`execute_interview`), manual-only (not in AVAILABLE_ACTIONS) on both sides.
- Channel ops: `step_env` → `SimEngine::run_with_boost` round loop producer (`sim/mod.rs:833`, fans out simulation_start/round_start/CREATE_POST/round_end/simulation_end to PlatformActionLogger); `interview_agent` → `execute_interview` (S-865 parity-verified); `create_model(use_boost)` → dual-LLM boost routing (twitter=llm, reddit=boost_llm); `close()` → IPC close_env + shutdown flag. Producer wait-for-commands loop in `run_sim_body` (rs:1597) services live interviews → resolves the prior `[!] IPC-PRODUCER-PENDING` (S-868, Cycle 56).
- `[≠]U028-OASIS-INTERNALS` (S-867): OASIS SQLite trace/post/user/comment DB enrichment — genuinely INEXPRESSIBLE without vendoring OASIS (contract explicitly: do NOT vendor OASIS internals). actions.jsonl RECORDS produced directly; only DB-internal `action_args` enrichment keys dropped. Survives the bar.
- NOTE: ledger SWEEP-3 row (parity-ledger.md:187) describes an OUTDATED "call Python OASIS via subprocess" decision; the implemented DECISION-14/U028 model (in-process inline LLM) supersedes it — the abstraction is realized in-process, not via subprocess.

### Overall: 4 REALIZED, **1 GAP (U-050 SIGNAL_CONTRACT)** → cycle-72 verdict **FAIL** (fail-closed).
The signal contract is the one no-downgrade violation: 2 of 3 signals (SIGTERM, SIGHUP) are unhandled and even SIGINT does not propagate sim cancellation. `cleanup_all` is landed but unwired. This is a portable feature skipped, not an inexpressible divergence — must be fixed (handler wiring), not waved through. The other four contracts faithfully map onto teri's tokio substrate with only non-contractual file-transport / OS-mechanism / OASIS-internal `[≠]`s that survive the `[≠]` bar.

---

## 2026-06-21 · Cycle 73 (FINAL re-verify) · U-050 SIGNAL_CONTRACT residual (atexit-net) fix

**Gate:** rust-port-parity-verifier (differential, adversarial — tried to REFUTE "the residual is closed AND nothing else is missing"). Both sides cited file:line. Build green (`cargo build` ✓), 1552 lib tests pass incl. `cleanup_all_is_idempotent`, `cleanup_all_stops_running_but_skips_finished`, `cleanup_all_preserves_finished_run_state`, `cleanup_all_terminates_and_records`.

### U-050 SIGNAL_CONTRACT → **REALIZED** (residual closed; flips cycle-72 GAP → PASS)

The 4 claims independently confirmed against `server.rs` (serve fn, lines read 263-354):
1. **serve result CAPTURED, not inline-`?`** — `let serve_result = axum::serve(...).with_graceful_shutdown(...).await;` (rs:307-341). No `?` on the serve await.
2. **post-serve cleanup BEFORE error propagation** — `sim_runner.cleanup_all().await;` (rs:349) runs on BOTH Ok and Err, signal or not, *before* `serve_result.map_err(TeriError::Io)?;` (rs:351). This is the atexit-net equivalent of `atexit.register(cls.cleanup_all_simulations)` (py:1343), covering the non-signal exit path (e.g. a serve error) that previously leaked RUNNING on-disk state.
3. **DISTINCT Arc clone** — `let sim_runner = state.sim_runner.clone();` (rs:282) + `let shutdown_runner = sim_runner.clone();` (rs:283). `shutdown_runner` is moved into the `async move` shutdown future (rs:339); `sim_runner` stays in scope for the post-serve call (rs:349). Both compile (build ✓) and both `Arc::clone` the SAME `Arc<SimulationRunner>` (`ApiState.sim_runner: Arc<…>`, api/mod.rs:298), so both reach the same instance.
4. **Idempotency safe** — `cleanup_done: AtomicBool` is a field on the single `SimulationRunner` instance (rs:1007), shared by both Arc clones. `cleanup_all` opens with `cleanup_done.compare_exchange(false, true, AcqRel, Acquire)` → early-return if already done (rs:1366-1373). So signal-path cleanup (rs:339) followed by post-serve cleanup (rs:349) = second call no-ops: no double-terminate, no state corruption. Test-backed (`cleanup_all_is_idempotent`).

### Three-signal path — NO REGRESSION
`server.rs:307-340`: SIGINT via `tokio::signal::ctrl_c()` (rs:311); SIGTERM via `SignalKind::terminate()` + SIGHUP via `SignalKind::hangup()` (`#[cfg(unix)]`, rs:316-327) select-joined into `terminate`; non-unix → `std::future::pending` (Ctrl-C only) — legit `[≠]` (no SIGTERM/SIGHUP on Windows). Outer `tokio::select!` (rs:333-336): both arms fall through to `shutdown_runner.cleanup_all().await` (rs:339) — no signal escapes cleanup. Matches X `register_cleanup` `has_sighup` guard + all-three→`cleanup_all_simulations` (py:1322-1353).

### Adversarial line-by-line vs X `register_cleanup` (py:1290-1358) — any OTHER missing behavior?
- **original-handler chaining / re-raise (py:1326-1340)** — after cleanup, X calls the saved `original_sigint/sigterm/sighup` (captured via `signal.getsignal`, py:1311-1317), else `sys.exit(0)` (SIGHUP, py:1337) or `raise KeyboardInterrupt` (SIG_DFL, py:1340). **Purpose (docstring py:1326 "让 Flask 正常退出"): plumbing to reach process exit** — Python handlers don't auto-terminate, so the original disposition must be re-invoked or the process hangs. **Non-observable `[≠]`, survives the bar:** in teri's tokio model the equivalent is structurally automatic — signal → shutdown future completes (after cleanup) → `axum::serve` returns → post-serve cleanup (no-op) → `serve()` returns → `main` returns → process exits 0 (or serve-error code). There is no "keep-running" hazard to guard, hence no handler to re-raise. No distinct exit code is owed: X's `sys.exit(0)` = exit 0 = teri's clean return; the `KeyboardInterrupt`/default-disposition branch is just the OS default "exit after cleanup," which teri also does. The only observable — "cleanup ran, then the process terminated" — is preserved on every signal.
- **subprocess runner `setup_signal_handlers` (scripts/run_twitter_simulation.py:749, SIGTERM/SIGINT→`_shutdown_event`, double-signal→`sys.exit(1)`)** — out of scope for the server-side server.rs contract; maps to the cooperative `shutdown` AtomicBool in `terminate_handle` (U-045, already REALIZED). In-process: no subprocess, no subprocess exit code (`[≠]` inexpressible, adjudicated under U-045).
- **debug/reloader gating (py:1299-1308, `WERKZEUG_RUN_MAIN`)** — Flask-reloader-only artifact; teri has no Werkzeug reloader. Non-contractual `[≠]`.
- **`_cleanup_registered` once-guard (py:1294-1297, 1358)** — teri installs handlers once inside the single `serve()` call; no re-registration path. Subsumed.

### Verdict
**U-050 → REALIZED.** Both the signal path AND the atexit-net (post-serve) path now faithfully map X `register_cleanup`. The cycle-72 GAP (SIGTERM/SIGHUP unhandled, cleanup unwired, non-signal exit leaking RUNNING) is fully closed. S-1059 flipped `- [x]`. No residual contractual behavior missing — the only X behaviors not mirrored byte-for-byte (original-handler chaining, subprocess exit codes, Werkzeug gating) are non-observable plumbing/out-of-substrate `[≠]`s that survive the `[≠]` bar. **Safe to flip the U-050 terminal.**

## Cycle 72 (35th resume) — runtime contracts + U-050 SIGNAL bug fix → symbol-map [ ]=0
- Adversarial parity-verifier on 5 runtime contracts: U-045 SUBPROCESS_ISOLATION, U-046 IPC_POLLING, U-049 BACKGROUND_THREAD, U-050 SIGNAL, SWEEP-3 OASIS.
- **GAP FOUND + FIXED (U-050 SIGNAL_CONTRACT):** teri's serve() installed only ctrl_c() (SIGINT) and never called cleanup_all → SIGTERM (kill/container/systemd default) would hard-kill running sims without persisting STOPPED. Fix (server.rs): SIGTERM+SIGINT+SIGHUP via tokio signal → cleanup_all; PLUS atexit-net (port of atexit.register, simulation_runner.py:1343) — cleanup_all also runs on the non-signal serve-return path (idempotent via cleanup_done). Two parity-verifier passes → REALIZED. 1552 tests pass, clippy/fmt clean.
- Adjudication: U-045/046/SWEEP-3 → [≠]SUBSTRATE (REALIZED via tokio/mpsc/native-SimEngine — OS process-groups/filesystem-IPC/OASIS-internals replaced by behavior-equivalent constructs); U-049 (shutdown handler extend-Y) + U-050 (LOCALE task-local, via U-005) + S-1059 (SIGNAL) → [x].
